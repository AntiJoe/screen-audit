...
for screenTag in ['FS', 'P', 'S', 'Combined']:
    ...
    ax = sns.regplot(x=df.RRm, y=df.CSFdrop, ci=None, label=screenTag, color=c[idx], 
...
plt.xlabel('Reject Rate by Mass', fontdict=font)
plt.ylabel('Freeness Drop (ml)', fontdict=font)
xpad = 5
ypad = 5
plt.xlim(min(dff.RRm) - xpad, max(dff.RRm) + xpad)
plt.ylim(min(dff.CSFdrop) - ypad, max(dff.CSFdrop) + ypad)
# plt.yticks(ticks=np.arr)
outfile = '{}-csfdrop-rrm.png'.format(screenTag)
